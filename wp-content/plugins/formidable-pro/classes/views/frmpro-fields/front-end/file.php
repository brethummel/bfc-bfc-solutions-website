...
if ( FrmField::is_read_only( $field ) ) {
	...
	foreach ( (array) $media_ids as $media_id ) {
		?>
<input type="hidden" <?php do_action( 'frm_field_input_html', $field ); ?> value="<?php echo esc_attr( $media_id ); ?>" name="<?php echo esc_attr( $input_name ); ?>" />
<div class="frm_show_file_icon"><?php echo FrmProFieldsHelper::get_file_icon( $media_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
	...
	?>
<input type="hidden" <?php do_action( 'frm_field_input_html', $field ); ?> name="<?php echo esc_attr( $input_name ); ?>" <?php echo $required_att; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> value="<?php echo esc_attr( $hidden_value ); ?>" data-frmfile="<?php echo esc_attr( $field['id'] ); ?>" />
						...
						<div class="dz-filename">
							<span data-dz-name="">
								<?php if ( ! empty( $file['accessible'] ) ) { ?>
									<a href="<?php echo esc_url( $file['file_url'] ); ?>" target="_blank" rel="noopener"><?php echo esc_html( $file['name'] ); ?></a>
								<?php } else { ?>
