...
class FrmProEntry {
	...
	public static function admin_edit_link( $id ) {
		return admin_url( 'admin.php?page=formidable-entries&frm_action=edit&id=' . absint( $id ) );
	}
	...
	public static function save_sub_entry( $field, $action, $new_values, &$values ) {
		...
		$old_ids      = $action === 'create' ? array() : self::get_existing_sub_entries( $values['id'], $field->id );
	...
	private static function get_existing_sub_entries( $entry_id, $section_id ) {
		...
		return $old_ids ? array_filter( (array) $old_ids, 'is_numeric' ) : array();
	}
	...
	public static function get_sub_entries( $entry_id, $meta = false ) {
		return FrmEntry::getAll( array( 'parent_item_id' => $entry_id ), '', '', $meta, false );
	}
	...
	private static function prepare_entries_query( &$query, &$args ) {
		...
		$order_fields = array();
		foreach ( $args['order_by_array'] as $o_key => $order_by_field ) {
			$order_fields[ $o_key ] = is_numeric( $order_by_field ) ? FrmField::getOne( $order_by_field ) : $order_by_field;
		}
