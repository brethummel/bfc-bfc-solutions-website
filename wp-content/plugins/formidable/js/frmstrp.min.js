...
"address_city");addValToRequest(addressContainer,prefix+"state",cardObject,"address_state");addValToRequest(addressContainer,prefix+"zip",cardObject,"address_zip")}};window.frmProForm.beforeConfirmPayment=function(params,meta){params.confirmParams.payment_method_data={billing_details:convertToAddressObject(meta)};return params};function convertToAddressObject(meta){var newMeta,k;newMeta={address:{}};for(k in meta){if(!meta.hasOwnProperty(k))continue;if(k==="address_zip")newMeta.address.postal_code=
meta[k];else if(k.indexOf("address_")===0)newMeta.address[k.replace("address_","")]=meta[k];else newMeta[k]=meta[k]}return newMeta}function addValToRequest(container,inputName,cardObject,objectName){var input=container.find('input[name$="['+inputName+']"]');if(input.length&&input.val())cardObject[objectName]=input.val()}window.frmProForm.submitButtonIsConditionallyDisabled=function(form){var formId=parseInt(form.querySelector('[name="form_id"]').value);return submitButtonIsConditionallyDisabled(formId)};
window.frmProForm.queryForNameField=function(nameID,type,form){if("container"===type)return document.querySelector("#frm_field_"+nameID+"_container, .frm_field_"+nameID+"_container");if(!form)return null;return form.querySelector("#frm_field_"+nameID+'_container input, input[name="item_meta['+nameID+'"], .frm_field_'+nameID+"_container input")};function listenForSubmitButtonMutations(form){var submitButton,observer;submitButton=form.querySelector(".frm_button_submit");if(!submitButton)return;observer=
